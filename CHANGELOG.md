...
- 项目转为由 `tursom` 接续维护的社区分支
- 重整用户、开发、贡献、架构和维护文档
- 校准社区发行渠道及首个维护版本发布前的状态说明
- 新增 B 站 HTTP 请求历史的脱敏持久化、限额保留和 Web 导出功能
- 新增 HLS 关键异常自动冻结、内容寻址正文存储和可重建 HTTP 现场包导出
- 新增 Windows x64、macOS arm64、Linux glibc/musl 多架构二进制和 GHCR 统一发布工作流
- 修复 HLS 音视频流顺序、init section 变化导致的异常分割和无限校验
- 修复 HLS 转封装失败，并在程序启动时自动封口可恢复的未完成录制
